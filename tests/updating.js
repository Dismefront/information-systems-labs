...
export default function () {
    ...
    if (!coordinatesList || coordinatesList.length === 0) {
        throw new Error('No coordinates available to update');
    }

    const coordinateToUpdate = coordinatesList[Math.floor(Math.random() * coordinatesList.length)];

    if (!coordinateToUpdate || !coordinateToUpdate.id) {
        throw new Error('Invalid coordinate data');
    }

    const updateResponse = http.put(
        `${BASE_URL}/api/coordinates/update/${coordinateToUpdate.id}`,
        JSON.stringify(coordinateToUpdate),
        { headers: getAllHeaders }
    );

    check(updateResponse, {
        'update coordinate succeeded': (res) => res.status === 200 || res.status === 204,
    });
}
