...
public class LocationController {
  ...
  public ResponseEntity update(
      @PathVariable long id, @RequestBody LocationRequest locationRequest, Principal principal) {
    String username = principal.getName();
    try {
      if (locationRequest.getZ() == null) {
        return ResponseEntity.badRequest().body("Z coordinate cannot be null");
      ...
      return ResponseEntity.ok()
          .body(locationService.updateLocation(locationRequest, username, id));
    } catch (Exception e) {
      return ResponseEntity.badRequest().body(e.getMessage());
    }
  }
  ...
  public ResponseEntity list(@RequestParam int page, @RequestParam int size, Principal principal) {
    ...
    return ResponseEntity.ok()
        .body(locationService.getLocationList(page, size, principal.getName(), isAdmin));
  }
}
