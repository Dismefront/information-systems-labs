...
public class RequestsController {
  ...
  public ResponseEntity findAll() {
    List<User> userList =
        requestRepository.findAllByStatus(RequestStatus.PENDING).stream()
            .map(x -> userRepository.getReferenceById(x.getUserId()))
            .toList();
  ...
  public ResponseEntity acceptRequest(@PathVariable long id) {
    ...
    requestRepository
        .findAllByUserIdAndStatus(user.getId(), RequestStatus.PENDING)
        .forEach(
            x -> {
  ...
  public ResponseEntity postRequest(@RequestBody AdminHttpRequest request) {
    List<AdminRequest> adminRequests =
        requestRepository.findAllByUserIdAndStatus(request.getUserId(), RequestStatus.PENDING);
