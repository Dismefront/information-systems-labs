...
public interface ProductRepository extends JpaRepository<Product, Long> {
  @Query(
      value =
          "SELECT new org.dismefront.data.product.ProductManaged(p.id, p.name, p.coordinates, p.creationDate, p.unitOfMeasure, "
      ...
      countQuery =
          "SELECT COUNT(p) FROM Product p LEFT JOIN Event e ON e.entityId = p.id AND e.name = :eventName")
  Page<ProductManaged> findProductsWithEditableFlag(
  ...
  @Query(
      value = "SELECT manufacturer_id, object_count FROM get_objects_count_by_manufacturer()",
      nativeQuery = true)
