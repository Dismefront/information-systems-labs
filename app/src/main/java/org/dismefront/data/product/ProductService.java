...
public class ProductService {
  ...
  private void logSaveEventsOfNestedObjects(Product product, String username) {
    ...
    if (!officialAddressHasId) {
      Event event =
          new Event(
              EventName.ADDRESS_CREATED,
              username,
              officialAddress.getId(),
              new Timestamp(new Date().getTime()));
      eventRepository.save(event);
    }
  ...
  public void uploadProductsFromFile(
      ArrayList<Map<String, Object>> products, String username, ImportHistory importHistory) {
    products.forEach(
        product -> {
  ...
  public void deleteProduct(Long id, String username) {
    ...
    Event event =
        new Event(EventName.PRODUCT_DELETED, username, id, new Timestamp(new Date().getTime()));
    eventRepository.save(event);
  }
  ...
  private void productParams(ProductRequest productRequest, Product product) {
    ...
    product.setCoordinates(
        coordinatesRepository.getReferenceById(productRequest.getCoordinatesId()));
    product.setCreationDate(ZonedDateTime.now());
    product.setUnitOfMeasure(productRequest.getUnitOfMeasure());
    product.setManufacturer(
        organizationRepository.getReferenceById(productRequest.getManufacturerId()));
  ...
  public Page<ProductManaged> getProductList(
      int page, int size, String username, boolean isAdmin, String sortBy, String sortDir) {
    Sort sort =
        sortDir.equalsIgnoreCase(Sort.Direction.ASC.name())
            ? Sort.by(sortBy).ascending()
            : Sort.by(sortBy).descending();
    Pageable pageable = PageRequest.of(page, size, sort);
    return productRepository.findProductsWithEditableFlag(
        username, EventName.PRODUCT_CREATED, isAdmin, pageable);
  }
  ...
  public List<ProductsByManufacturersCountResponse> getProductsGroupedByManufacturer() {
    ...
    dbRes.forEach(
        (product) -> {
          products.add(
              new ProductsByManufacturersCountResponse(
                  organizationRepository.getReferenceById((Long) product[0]), (Long) product[1]));
        });
    return products;
  }
  ...
  public List<Product> getProductsByManufacturer(int manufacturerId) {
    ...
    dbRes.forEach(
        (product) -> {
          products.add(
              new Product(
                  Long.valueOf((Integer) product[0]),
                  (String) product[1],
                  coordinatesRepository.getReferenceById((Long) product[7]),
                  ZonedDateTime.now(),
                  UnitOfMeasure.valueOf((String) product[6]),
                  organizationRepository.getReferenceById((Long) product[8]),
                  (long) product[3],
                  (float) ((double) product[4]),
                  (int) product[5],
                  (String) product[2],
                  null));
        });
