...
public class AddressService {
  ...
  public void deleteAddress(Long id, String username) {
    ...
    Event event =
        new Event(EventName.ADDRESS_DELETED, username, id, new Timestamp(new Date().getTime()));
    eventRepository.save(event);
  }

  public Page<AddressManaged> getAddressList(int page, int size, String username, boolean isAdmin) {
    Pageable pageable = PageRequest.of(page, size);
    return addressRepository.findAddressesWithEditableFlag(
        username, EventName.ADDRESS_CREATED, isAdmin, pageable);
  }
}
